...
const dictionaryCssFiles = ["hint/hint", "index", "page/page"];

function displayEventOptions() {
  console.clear();
  console.log(
    `${BGgreen}***  Please select which event you would like to build ***${resetStyle}\n`
  );
...
function handleEventSelection() {
  console.clear();
  console.log(
    `${BGgreen}***  ${selectedEvent.name} - Please select a property to build ***${resetStyle}\n`
  );
...
// Define a function to replace placeholders in a script
const replaceScriptPlaceholder = (script, placeholder, value) => {
  const regex = new RegExp(`\\b${placeholder}\\b`, "g");
  return script.replace(regex, value);
};
...
function writeChoice() {
  ...
  //change the app.js according to the required content
  ...
  //REMOVING the function to avoid unnecessary images
      ...
      if (Array.isArray(config) && config.length > 0 && config[0].event) {
        ...
        fs.readFile(packageFile, "utf8", (err, packageData) => {
          ...
          try {
            ...
            const scriptsToUpdate = ["build", "scss"];
            packageJson.homepage = homepage;
            scriptsToUpdate.forEach((scriptName) => {
              const originalScript = packageJson.scripts[scriptName];

              // Replace basename placeholder
              const regexBuild = /(--basename\s+)[^\s]+/g;
              const updatedBuildScript = originalScript.replace(
                regexBuild,
                `--basename ${selectedEvent.name}`
              );

              // Replace components placeholder
              const scssPathComponent =
                dictionaryCssFiles[dictionaryOptions[selectedOption]] + ".scss";
              const regexScss = /\/components\/([^\/]+)\/[^-]+/;
              let updatedScssScript = updatedBuildScript.replace(
                regexScss,
                `/components/${selectedEvent.name.toLowerCase()}/${scssPathComponent} `
              );
              packageJson.scripts[scriptName] = updatedScssScript;
            });
  ...
  console.log(`
  ${BGgreen}***  ${selectedEvent.name}...building ${selectedOption}  ***${resetStyle}\n
  `);
  rl.close();
}
